...
public:
   ...
   RotationMode getMode (                 ) const;
	...
	void Rotate   (Vector axis, double angle);	// rotate around arbitrary axis
	void Rotate   (AffineMap mat            );	// rotate by rotation matrix mat
	void Rotate   (Quaternion q             );	// rotate by quaternion quat
	void Translate(Vector vec               );	// translate by vec
